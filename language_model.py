...
class Model(object):
    ...
    def back_propagate(self, input_batch, activations, loss_derivative):
        ...
        ###########################   YOUR CODE HERE  ##############################
        hid_to_output_weights_grad = np.transpose(np.dot(np.transpose(activations.hidden_layer),loss_derivative))
        output_bias_grad = np.sum(loss_derivative,axis=0)/(loss_derivative.shape[0])
        embed_to_hid_weights_grad = np.transpose(np.dot(np.transpose(activations.embedding_layer),hid_deriv))
        hid_bias_grad = np.sum(hid_deriv,axis=0)/(hid_deriv.shape[0])
